...
export const RenderNewsArticles = ({data}) => {
    const renderData = () => data?.map((item, idx) => <RenderArticle key={item.title} item={item} idx={idx} />)
  return (
    <section className='masonry' style={{columnCount: 3}}>
        {renderData()}
    </section>
  )
}
...
const RenderArticle = ({item, idx}) => {
    ...
    return (
        <article className='w-1/4 text-xl'>
            ...
            <button className='bg-blue-600 p-1 px-2'><a href={link}>Click Here To Read From Source Site</a></button>
        </article>
    )
}
